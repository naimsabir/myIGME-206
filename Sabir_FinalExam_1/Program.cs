    ...
    class Program
    {
        ...
        static int[] stateList = new int[]
       {
        ...
        static void Q3()
        {
            ...
            {
                "You wake up in a dark cell with light protruding from one barred window that reveals a snowy mountain range with lights adorning the peaks." +
                "\n You stare out the window when your cell door mysteriously opens and you see two halls (A & C) to go down.",

                "You walk in to what seems to be a lounge for the guards that is now deserted. There's a door which leads to another cell block (D) \nand then there's " +
                "a locked door to the warden's office (F) that looks just weak enough to force through ",

                "You enter what looks like a mess hall that's covered in bloodstains and abandoned weapons. The hall continues into another cell block on one end (D) \n" +
                "but there is also a huge door on the other end (E)",

                "You enter an empty cell block that's completely devoid of any signs of life. In fact, it's weirdly clean for an abandoned prison. \n" +
                "There's a door that leads to a lounge (B) and door that leads to a a large room (C)",

                "You enter an armory that's mostly empty save for a few helmets and swords scattered about the place. There is the door to the mess hall behind you (C)" +
                "and an ominous dark hallway in front of you (G)",

                "You enter the warden's office and immediately hear the doors lock behind you leaving only a hallway that leads to the frozen wilderness outside the prison (H).",

                "You enter a hallway that is pitch black. You can barely see in front of you but you can tell there's a doorway ahead of you (F). \n" +
                "However, you could always turn back (E)",
            ...
            Console.WriteLine($"There are 8 room labeled A-H and tje states of the room change every second from ice -> liquid -> gas -> liquid -> ice etc" +
                $" their current states are respectively: ");
            for(int i = 0; i < stateList.Length; i++)
            {
                NodeState state = (NodeState)stateList[i];
                Console.WriteLine($"{state}");
            }

            // ask player to start the game
            Console.WriteLine("Would you like to start the game?");
            willStart = Console.ReadLine().ToLower();
            if(willStart.StartsWith("y"))
            {
                goto start;
            }
            else
            ...
            while (nRoom != 7)
            {
                ...
                NodeState playState = (NodeState)playerState;
                Console.WriteLine($"The player state is: {playState}");
                    ...
                    while (!bValid)
                    {
                        ...
                        if (bValid)
                        {
                            lock (lockObject)
                            {
                                ...
                                {
                                    Console.WriteLine("The states were not the same.");
                                }
                            }
                        }
        ...
        public class SettingsClass : IPlayerSettings
        {
            ...
            public void SavePlayerSettings(string fileName, PlayerSettings settings)
            {
                ...
                fileName = JsonConvert.SerializeObject(sSettings);
            }
